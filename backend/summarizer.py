...
from downloader import BILIBILI_HEADERS, is_bilibili_url
from iqiyi import IqiyiParser, is_iqiyi_url
from site_775069 import is_775069_url
...
class SubtitleExtractor:
    ...
    def extract(self, url: str) -> dict:
        ...
        """
        if is_775069_url(url) or is_douyin_url(url):
            return {
                "has_subtitle": False,
                "language": "",
                "subtitle_type": "unsupported",
                "segments": [],
                "full_text": "",
            }
        ...
        if is_iqiyi_url(url):
            return self._extract_iqiyi(url)
    ...
    def _extract_iqiyi(self, url: str) -> dict:
        """Extract public iQiyi subtitles without invoking yt-dlp."""
        empty = {
            "has_subtitle": False,
            "language": "",
            "subtitle_type": "none",
        ...
        tracks = self.iqiyi.get_subtitles(url)
        if not tracks:
            return empty

        track = next(
            (
                item for language in self.PREFERRED_LANGS
                for item in tracks
                if item["language"] == language and item["ext"] == "vtt"
            ),
            next((item for item in tracks if item["ext"] == "vtt"), tracks[0]),
        )
        response = httpx.get(
            track["url"],
        ...
        if track["ext"] != "vtt":
            raise ValueError("爱奇艺当前仅返回了暂不支持的字幕格式")
        segments = self._parse_vtt_content(response.text)
        if not segments:
            return empty
    ...
    def _parse_vtt(filepath: str) -> list[dict]:
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            content = f.read()
        return SubtitleExtractor._parse_vtt_content(content)

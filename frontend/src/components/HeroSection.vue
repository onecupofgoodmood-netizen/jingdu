  ...
  >
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
      ...
      <div class="max-w-2xl mx-auto">
        <form @submit.prevent="onSubmit" class="relative flex items-center" role="search" :aria-label="t('hero.searchLabel')">
          <div class="relative flex-1">
            <label for="video-url-input" class="sr-only">{{ t('hero.inputLabel') }}</label>
        ...
        <div v-if="showSlogan" class="flex flex-wrap items-center justify-center gap-3 mt-5 text-xs text-text-muted">
          ...
          <button
            ...
            :disabled="loading"
            :aria-label="`${t('hero.try')} ${example.label}`"
            class="px-3 py-1 rounded-full bg-white border border-border-light hover:border-primary hover:text-primary transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {{ example.label }}
          </button>
        </div>
...
const examples = [
  ...
  { label: 'Bilibili', url: 'https://www.bilibili.com/video/BV1eS4y157Ey' },
  { label: 'Twitter/X', url: 'https://x.com/historyinmemes/status/1790637656616943991' },
]
...
function useExample(example) {
  ...
  emit('parse', normalizeUrl(example.url))
}
</script>

  ...
  <div class="bg-white rounded-2xl border border-border shadow-lg overflow-hidden h-full flex flex-col">
        ...
        <div class="p-5 sm:p-6 min-h-[400px] flex-1 overflow-y-auto">
          ...
          <div v-show="activeTab === 'summary'">
            <div
              v-if="!user && !loading && !summaryText"
              class="flex flex-col items-center justify-center py-14 text-center"
            >
              <div class="w-12 h-12 mb-4 rounded-full bg-primary-light text-primary flex items-center justify-center">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                    d="M15 7a3 3 0 11-6 0 3 3 0 016 0zm-9 13a6 6 0 0112 0" />
                </svg>
              </div>
              <p class="text-sm text-text-secondary mb-4">{{ t('summary.loginPrompt') }}</p>
              <button
                type="button"
                @click="emit('need-login')"
                class="h-10 px-6 rounded-full bg-primary hover:bg-primary-dark text-white text-sm font-medium transition-colors cursor-pointer"
              >
                {{ t('common.login') }}
              </button>
            </div>
            ...
            <div v-if="quotaInfo && quotaInfo.remaining >= 0 && !loading" class="mt-4 p-3 rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-between">
              <span class="text-sm text-blue-700">
                {{ t('summary.quota') }}<strong>{{ quotaInfo.remaining }}</strong> / {{ quotaInfo.limit }}
              </span>
              ...
              <div class="flex items-center justify-between mb-4">
                <div class="text-sm text-text-secondary">
                  ...
                  <span v-if="subtitleData.language" class="ml-2 px-2 py-0.5 bg-primary-light text-primary rounded-full text-xs">
                    {{ subtitleData.subtitle_type === 'manual' ? t('summary.manualSubtitle') : t('summary.autoSubtitle') }} · {{ subtitleData.language }}
                  </span>
                </div>
                <div class="flex items-center gap-3">
                  ...
                  >
                    {{ subtitleExpanded ? t('summary.collapse') : t('summary.expandAll') }}
                  </button>
                </div>
              </div>
              <div
          ...
          <div v-show="activeTab === 'mindmap'">
            <div v-if="mindmapMarkdown" class="relative">
              ...
              <div class="flex items-center justify-end gap-2 mb-3">
                ...
                <button
                  ...
                  class="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg text-primary hover:bg-primary-light transition-colors cursor-pointer"
                  :title="isFullscreen ? t('summary.exitFullscreen') : t('summary.fullscreen')"
                >
            ...
            <div v-else-if="loading" class="flex flex-col items-center justify-center py-16">
              <div class="w-10 h-10 border-4 border-primary/20 border-t-primary rounded-full animate-spin mb-3"></div>
              <p class="text-text-muted text-sm">{{ t('summary.generatingMindmap') }}</p>
            </div>
            <div v-else class="flex flex-col items-center justify-center py-16 text-text-muted">
              ...
              <p class="text-sm">{{ t('summary.mindmapEmpty') }}</p>
            </div>
          </div>
            ...
            <div class="space-y-4">
              ...
              >
                <div v-if="chatMessages.length === 0" class="flex flex-col items-center justify-center py-12 text-text-muted">
                  ...
                  <p class="text-sm mb-1">{{ t('summary.qaPrompt') }}</p>
                  <p class="text-xs">{{ t('summary.qaExample') }}</p>
                </div>
                <div
              ...
              <div class="flex gap-2 pt-3 border-t border-border-light">
                <input
                  ...
                  type="text"
                  :placeholder="t('summary.questionPlaceholder')"
...
const tabs = computed(() => t('summary.tabs').map((label, index) => ({ ...tabMeta[index], label })))

const activeTab = ref('summary')
const loading = ref(false)
const loadingMessage = ref(t('summary.extracting'))
...
const subtitleFormats = computed(() => {
  const labels = t('summary.subtitleFormats')
  return [
    { key: 'srt', label: labels[0], ext: 'srt' },
    { key: 'vtt', label: labels[1], ext: 'vtt' },
    { key: 'txt', label: labels[2], ext: 'txt' },
  ]
})
...
watch(() => props.user, (user, previousUser) => {
  if (user && !previousUser && !loading.value && !summaryText.value) {
    startSummarize()
  }
})
...
function renderMindmap(md) {
  ...
  try {
    ...
    console.warn('Mind map render failed:', e)
  }
}
  ...
  return new Promise((resolve) => {
    img.onload = () => {
      ...
      canvas.toBlob((pngBlob) => {
        if (pngBlob) {
          triggerDownload(pngBlob, `${getSafeFilename()} - ${t('summary.mindmapFile')}.png`)
        }
        resolve()
      }, 'image/png')
    }
    img.onerror = () => {
      URL.revokeObjectURL(url)
      alert(t('summary.exportPngFailed'))
      resolve()
    }
    img.src = url
  })
...
function downloadMindmapSvg() {
  ...
  const blob = new Blob([svgString], { type: 'image/svg+xml;charset=utf-8' })
  triggerDownload(blob, `${getSafeFilename()} - ${t('summary.mindmapFile')}.svg`)
}
...
function downloadSubtitle(format) {
  ...
  triggerDownload(blob, `${filename} - ${t('summary.subtitleFile')}.${ext}`)
}
...
function getSafeFilename() {
  return (props.videoTitle || t('summary.videoFallback')).replace(/[\\/*?:"<>|]/g, '_').substring(0, 80)
}
...
async function startSummarize() {
  ...
  loadingMessage.value = t('summary.extracting')

  try {
    await summarizeVideo(props.videoUrl, locale.value, {
      subtitle: (data) => {
        try {
      ...
      error: (data) => {
        ...
        try {
          ...
          alert(parsed.message || t('summary.summaryFailed'))
        } catch (e) {
          alert(t('summary.summaryFailed') + ': ' + data)
        }
      },
    })
  } catch (err) {
    loading.value = false
    alert(t('summary.requestFailed') + ': ' + err.message)
  }
}
...
async function sendQuestion() {
  ...
  try {
    await chatWithVideo(
      ...
      subtitleData.value.full_text || '',
      {
        ...
        error: (data) => {
          ...
          try {
            const parsed = JSON.parse(data)
            aiMessage.content = '❌ ' + (parsed.message || t('summary.answerFailed'))
          } catch (e) {
            aiMessage.content = '❌ ' + t('summary.answerFailed')
          }
        },
      }
    )
    ...
    aiMessage.loading = false
    chatLoading.value = false
    aiMessage.content = '❌ ' + t('summary.requestFailed') + ': ' + err.message
  }
}

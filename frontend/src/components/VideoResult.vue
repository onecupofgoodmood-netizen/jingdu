    ...
    <div class="border-t border-border-light px-5 sm:px-6 py-5">
        ...
        >
          ...
          <div class="min-w-0">
            ...
            <div class="text-xs text-text-muted">
              {{ fmt.ext.toUpperCase() }} · {{ fmt.is_audio_only ? t('video.audioOnly') : fmt.has_audio ? t('video.hasAudio') : t('video.videoOnly') }}
            </div>
          </div>
        </button>
      </div>
      ...
      <div class="mt-6 flex flex-col items-stretch gap-3">
        ...
        >
          ...
          {{ downloading ? t('video.downloading') : t('video.downloadNow') }}
        </button>
        <!-- AI 总结按钮（重新总结） -->
        <button
        ...
        >
          ...
          {{ summarizing ? t('video.summarizing') : t('video.aiSummary') }}
        </button>
        <span v-if="selectedFormat" class="text-xs text-text-muted text-center">
          {{ t('video.selected', { label: getSelectedLabel() }) }}
        </span>
      </div>
    </div>
  </div>
...
function formatViewCount(count) {
  if (!count) return ''
  if (locale.value === 'zh') {
    if (count >= 100000000) return (count / 100000000).toFixed(1) + t('video.viewsYi')
    if (count >= 10000) return (count / 10000).toFixed(1) + t('video.viewsWan')
  }

<template>
  <header class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-border-light">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      <a href="/" class="flex items-center gap-3" :title="t('common.appTitle')">
        <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center shadow-sm" role="img" :aria-label="`${t('common.brand')} Logo`">
        ...
        <span class="text-base sm:text-lg font-semibold text-text-primary tracking-tight">{{ t('common.brand') }}</span>
        <span class="hidden sm:inline text-xs text-text-muted bg-primary-light px-2 py-0.5 rounded-full">{{ t('common.videoDownloader') }}</span>
      </a>
      <nav class="hidden md:flex items-center gap-6 text-sm text-text-secondary" :aria-label="t('header.navLabel')">
        <a href="#features" class="hover:text-primary transition-colors">{{ t('header.features') }}</a>
        <a href="#how-to-use" class="hover:text-primary transition-colors">{{ t('header.howTo') }}</a>
        <a href="#comparison" class="hover:text-primary transition-colors">{{ t('header.comparison') }}</a>
        <a href="#pricing" class="hover:text-primary transition-colors">{{ t('header.pricing') }}</a>
      </nav>
      <div class="flex items-center gap-3">
        ...
        <select
          ...
          class="h-9 rounded-full border border-border-light bg-white px-3 text-sm text-text-secondary hover:border-primary/50 focus:outline-none focus:ring-2 focus:ring-primary/20"
        >
            ...
            <div v-if="menuOpen" class="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl border border-border shadow-xl py-2 animate-menu-in">
              <div class="px-4 py-2 border-b border-border">
                ...
                <p class="text-xs text-text-muted mt-0.5">
                  {{ user.is_vip ? t('common.vipMember') : t('common.freeUser') }}
                  <span v-if="user.is_vip && user.vip_expire_at" class="ml-1">· {{ t('common.expires', { date: formatDate(user.vip_expire_at) }) }}</span>
                </p>
              </div>
...
defineEmits(['login', 'register', 'logout', 'open-vip', 'change-locale'])

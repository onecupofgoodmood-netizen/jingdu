      ...
      <div v-if="paymentToast" class="fixed top-20 left-1/2 -translate-x-1/2 z-[200] animate-toast-in">
        <div :class="[
          ...
          <span class="font-medium text-sm">
            {{ paymentToast === 'success' ? t('app.paymentSuccess') : t('app.paymentCancel') }}
          </span>
        </div>
      </div>
    </Teleport>
...
import { createCheckoutSession, confirmPayment } from './api/payment.js'
import { createI18n, provideI18n } from './i18n.js'
...
async function handleOpenVip() {
  ...
  try {
    ...
    const msg = err.response?.data?.detail || err.message || t('app.checkoutFailed')
...
async function handleParse(url) {
  ...
  try {
    ...
    if (res.success) {
      ...
      alert(t('app.parseFailed') + (res.error || t('app.unknownError')))
    }
  } catch (err) {
    const msg = err.response?.data?.detail?.error || err.response?.data?.detail || err.message
    alert(t('app.parseFailed') + msg)
  } finally {
    loading.value = false
  }
}
...
async function handleDownload(formatId) {
  ...
  try {
    ...
    alert(t('app.downloadFailed') + (err.message || t('app.retryLater')))
  } finally {
